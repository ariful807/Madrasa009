...
import { DynamicSettingsEditor } from '../components/admin/DynamicSettingsEditor';
import { DynamicMarksEntry } from '../components/admin/DynamicMarksEntry';
...
export function AdminView({
  ...
  const handleTestSync = async () => {
    ...
    setIsSyncing(true);
    setSyncStatus('গুগল শিটের সাথে তথ্য সিঙ্ক হচ্ছে...');
    try {
  ...
  const handleAddResult = (e: FormEvent) => {
    ...
    const fallbackSubjects: SubjectMark[] = [
      { subjectName: 'হিফজুল কুরআন / মূল কিতাব', fullMark: 100, passMark: 33, obtainedMark: Math.min(100, Math.round(Number(newResult.obtainedTotal) * 0.22)), grade: 'A+' },
      { subjectName: 'তাজবীদ ও মাখরাজ', fullMark: 100, passMark: 33, obtainedMark: Math.min(100, Math.round(Number(newResult.obtainedTotal) * 0.20)), grade: 'A+' },
      { subjectName: 'আকাইদ ও ফিকহ', fullMark: 100, passMark: 33, obtainedMark: Math.min(100, Math.round(Number(newResult.obtainedTotal) * 0.20)), grade: 'A' },
      { subjectName: 'আরবি ভাষা ও সাহিত্য', fullMark: 100, passMark: 33, obtainedMark: Math.min(100, Math.round(Number(newResult.obtainedTotal) * 0.19)), grade: 'A' },
      { subjectName: 'বাংলা, গণিত ও ইংরেজি', fullMark: 100, passMark: 33, obtainedMark: Math.min(100, Math.round(Number(newResult.obtainedTotal) * 0.19)), grade: 'A' }
    ];
    ...
    const resultItem: StudentResult = {
      ...
      fatherName: newResult.fatherName,
      ...
      gpa: newResult.gpa || '5.00 (A+)',
      division: (newResult.division || 'মুমতাজ (স্টার)') as any,
      publishedDate: new Date().toLocaleDateString('bn-BD'),
      subjects: newResult.subjects && newResult.subjects.length > 0 ? newResult.subjects : fallbackSubjects
    };
      ...
      <div className="bg-gradient-to-r from-emerald-950 to-teal-950 rounded-2xl p-6 text-white shadow-lg flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        ...
        <div className="flex flex-wrap items-center gap-2.5">
          <button
            ...
            disabled={isSyncing}
            className="px-3.5 py-2 bg-emerald-800 hover:bg-emerald-700 text-white rounded-xl text-xs font-semibold flex items-center gap-1.5 transition-colors border border-emerald-600 disabled:opacity-60"
            id="admin-sync-btn"
          >
            <RefreshCw className={`w-3.5 h-3.5 ${isSyncing ? 'animate-spin text-amber-300' : ''}`} />
            <span>{isSyncing ? 'সিঙ্ক হচ্ছে...' : 'গুগল শিট সিঙ্ক'}</span>
          </button>
      ...
      {adminTab === 'settings' && (
        <div className="bg-white rounded-2xl p-6 sm:p-8 border border-slate-200 shadow-xs space-y-8">
          ...
          <form onSubmit={handleSaveSettings} className="space-y-8">
            ...
            {/* 10. Dynamic Goals, Principles, Rules, Admission Form Fields & Features */}
            <div className="space-y-4 pt-4 border-t border-slate-200/80">
              <div className="flex items-center gap-2">
                <Layers className="w-4 h-4 text-emerald-800" />
                <h3 className="text-sm font-bold text-slate-900 font-heading">
                  ১০. লক্ষ্য ও উদ্দেশ্য, মূলনীতি, আচরণবিধি, বৈশিষ্ট্য ও অনলাইন ভর্তি ফরম ফিল্ড
                </h3>
              </div>
              <p className="text-xs text-slate-500">
                এই অংশ থেকে আপনি মাদরাসার লক্ষ্য, মূলনীতি, ছাত্রদের নিয়মাবলী, মাদরাসার বৈশিষ্ট্য এবং ভর্তি ফরমের ফিল্ডসমূহ ডায়নামিকভাবে যোগ, এডিট বা ডিলিট করতে পারবেন। এখানে যেকোনো পরিবর্তন করা হলে পুরো সাইটে এবং গুগল শিটে সাথে সাথে আপডেট হবে।
              </p>
          ...
          {showResultForm && (
            <form onSubmit={handleAddResult} className="bg-slate-50 p-5 rounded-2xl border border-slate-200 space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                ...
                <div>
                  <label className="block text-xs font-semibold text-slate-700 mb-1">মেধা স্থান / পজিশন (ঐচ্ছিক)</label>
                  <input
                    type="text"
                    value={newResult.position}
                    onChange={e => setNewResult({ ...newResult, position: e.target.value })}
                    placeholder="যেমন: ১ম স্থান / মেধা তালিকায় ৯ম"
                    className="w-full px-3 py-2 rounded-lg border border-slate-300 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-600 bg-white"
                  />
                </div>
              </div>

              {/* Dynamic Marks Entry Component */}
              <div className="pt-2">
                <DynamicMarksEntry
                  jamaats={jamaats}
                  resultData={newResult}
                  onChange={(updated) => setNewResult(updated as any)}
                />
              </div>
              ...
              <div className="flex justify-end pt-2">
                <button
                  type="submit"
                  className="px-6 py-2.5 bg-emerald-800 text-white font-bold text-xs rounded-xl shadow-sm hover:bg-emerald-700 transition-colors flex items-center gap-1.5"
                >
                  <Save className="w-4 h-4" />
