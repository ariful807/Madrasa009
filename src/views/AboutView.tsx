...
import { DEFAULT_GOALS, DEFAULT_PRINCIPLES, DEFAULT_FEATURES } from '../data/defaultData';
...
export function AboutView({ settings, onNavigate, teachers: propsTeachers }: AboutViewProps) {
  ...
  const goalsList = settings.goals && settings.goals.length > 0 ? settings.goals : DEFAULT_GOALS;
  const principlesList = settings.principles && settings.principles.length > 0 ? settings.principles : DEFAULT_PRINCIPLES;
  const featuresList = settings.features && settings.features.length > 0 ? settings.features : DEFAULT_FEATURES;
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-12">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        <div className="lg:col-span-8 bg-white p-6 sm:p-8 rounded-2xl border border-slate-200 shadow-xs space-y-5">
          ...
          <div className="prose prose-slate text-sm sm:text-base text-slate-700 leading-relaxed space-y-4">
            <p>
              আল্লাহ তায়ালার অশেষ মেহেরবানীতে দ্বীনি ইলমের আলো ছড়িয়ে দিতে এবং একটি যুগোপযোগী আদর্শ ইসলামী শিক্ষা প্রতিষ্ঠান গড়ে তোলার মহৎ উদ্দেশ্যে <strong>{settings.establishedDate}</strong> তারিখে ঢাকার পশ্চিম যাত্রাবাড়ীতে মারকাযুল ইহসানের আনুষ্ঠানিক শিক্ষা কার্যক্রমের শুভ সূচনা হয়।
            </p>
            <p>
          ...
          {/* Dynamic Goals & Principles from SiteSettings */}
          <div className="pt-4 border-t border-slate-100 grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="bg-emerald-50/70 p-4 rounded-xl border border-emerald-100 space-y-2">
              <div className="flex items-center gap-1.5 text-xs font-bold text-emerald-900 uppercase">
                ...
                <span>আমাদের মূল লক্ষ্য ও উদ্দেশ্য:</span>
              </div>
            ...
            <div className="bg-amber-50/70 p-4 rounded-xl border border-amber-100 space-y-2">
              <div className="flex items-center gap-1.5 text-xs font-bold text-amber-900 uppercase">
                ...
                <span>আমাদের মূল আদর্শ ও পথচলা:</span>
              </div>
              <ul className="space-y-1.5 text-xs sm:text-sm text-slate-700">
        ...
        <div className="lg:col-span-4 bg-white p-6 rounded-2xl border border-slate-200 shadow-xs space-y-4">
          ...
          <div className="space-y-3 text-xs sm:text-sm">
            ...
            <div>
              ...
              <strong className="text-slate-800">বেফাকুল মাদারিসিল আরাবিয়া বাংলাদেশ ও আল-হাইআতুল উলয়া</strong>
            </div>
            <div>
              <span className="text-slate-500 block text-xs">ক্যাম্পাসসমূহ:</span>
              <p className="text-slate-700 mt-0.5">
                {settings.hasTemporaryCampus !== false && (
                  <>১. {settings.addressTemporary || 'পশ্চিম যাত্রাবাড়ী (অস্থায়ী)'}<br /></>
                )}
                ২. {settings.addressPermanent || 'পাইটি, ডেমরা (স্থায়ী)'}
              </p>
            </div>
          </div>
          ...
          <div className="pt-3 border-t border-slate-100 flex flex-col gap-2">
            <button
              ...
              className="w-full py-2.5 bg-emerald-800 hover:bg-emerald-700 text-white font-semibold text-xs rounded-lg transition-colors cursor-pointer"
            >
              ভর্তি তথ্য দেখুন
            </button>
            <button
              onClick={() => onNavigate('rules')}
              className="w-full py-2 bg-slate-100 hover:bg-slate-200 text-slate-800 font-semibold text-xs rounded-lg transition-colors cursor-pointer"
            >
              শিক্ষার্থী নীতিমালা ও রুটিন
            </button>
          </div>
        </div>
      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-8 items-center">
          <div className="md:col-span-4 text-center">
            ...
            <p className="text-xs text-slate-500">
              {settings.founderDesignation || 'প্রতিষ্ঠাতা, পরিচালক ও মোতাওয়াল্লী'}
            </p>
          </div>
          ...
          <div className="md:col-span-8 bg-slate-50 p-6 rounded-xl border border-slate-200 text-slate-700 text-sm sm:text-base leading-relaxed space-y-3">
            <p className="italic text-slate-800 font-serif">
              {settings.principalMessage || `"বিসমিল্লাহির রাহমানির রাহীম। সমস্ত প্রশংসা মহান রব্বুল আলামীনের, যিনি আমাদেরকে দ্বীনের খাদেম হিসেবে কবুল করেছেন। মারকাযুল ইহসান কোনো সাধারণ শিক্ষালয় নয়; এটি একটি আদর্শিক পরিবার। আমরা চেষ্টা করি প্রতিটি ছাত্রের অন্তরে তাকওয়া, বিনয়, আল্লাহর ভয় এবং সুন্নাহর গভীর মহব্বত প্রতিষ্ঠা করতে।"`}
            </p>
            <p>
      ...
      {/* 3. সম্মানিত শিক্ষক ও উস্তাদমণ্ডলীর পরিচিতি */}
      <TeacherProfileSection teachers={teachers} />

      {/* 4. বিশেষ বৈশিষ্ট্যসমূহ (Dynamic Features from SiteSettings) */}
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {featuresList.map((item, idx) => (
            <div key={item.id || idx} className="bg-white p-5 rounded-xl border border-slate-200 shadow-xs flex flex-col justify-between">
              <div>
