...
export const storageService = {
  ...
  saveSettings(settings: SiteSettings): void {
    ...
    if (url) {
      this.sendPostToGas(url, 'saveSettings', settings).catch(err => {
        console.warn('Auto sync settings to Google Sheets failed:', err);
      });
    }
  },
  ...
  saveNotices(notices: NoticeItem[]): void {
    ...
    const url = settings.googleSheetWebAppUrl || settings.googleAppsScriptUrl;
    if (url) {
      this.sendPostToGas(url, 'syncAllFromAdmin', { notices }).catch(err => {
        console.warn('Auto sync notices to Google Sheets failed:', err);
      });
    }
  },
  ...
  saveResults(results: StudentResult[]): void {
    ...
    const url = settings.googleSheetWebAppUrl || settings.googleAppsScriptUrl;
    if (url) {
      this.sendPostToGas(url, 'syncAllFromAdmin', { results }).catch(err => {
        console.warn('Auto sync results to Google Sheets failed:', err);
      });
    }
  },
