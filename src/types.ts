export type NavigationTab = 
  ...
  | 'blog'
  | 'admin'
  // Legacy / shortcut tabs that route into about sections
  | 'campus'
  | 'rules'
  | 'khidmat-fund'
  | 'contact';
...
export interface AdmissionFormField {
  id: string;
  label: string;
  type: 'text' | 'number' | 'tel' | 'date' | 'select' | 'textarea' | 'payment_method';
  required: boolean;
  placeholder?: string;
  options?: string[];
  enabled: boolean;
...
export interface SliderImageItem {
  id: string;
  title?: string;
  caption?: string; // Centered caption in slider footer
  subtitle?: string;
  imageUrl: string;
  badge?: string;
...
export interface TeacherItem {
  ...
  experience?: string; // শিক্ষক এর অভিজ্ঞতা বা বিবরণ
  phone?: string; // মোবাইল নম্বর (ঐচ্ছিক)
  email?: string;
  imageUrl?: string; // গোল প্রোফাইল ইমেজ
  bio?: string;
  order?: number;
  isActive?: boolean;
}

export interface JamaatBook {
  id: string;
  name: string;
  fullMark: number;
  passMark: number;
...
export interface JamaatItem {
  ...
  monthlyFee?: number; // মাসিক বেতন
  monthlyTuitionFee?: number;
  foodFee?: number; // খাবার ফি
  description?: string;
  isActive?: boolean;
  books?: JamaatBook[]; // কিতাব ও বিষয় তালিকা
}
